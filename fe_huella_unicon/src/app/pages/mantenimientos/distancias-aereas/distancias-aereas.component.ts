...
@Component({
  selector: 'app-distancias-aereas',
  templateUrl: './distancias-aereas.component.html',
  styleUrls: ['./distancias-aereas.component.css']
})
export class DistanciasAereasComponent implements OnInit {
  ...
  async fnListarAereas() {
    try {
      this.fShowSkeleton = true;
      let data: IDataResponse = await lastValueFrom(this.distanciaService.listarPorTipo('A'));
      if (data.exito) {
        this.lstAereas = data.datoAdicional;
        //console.log('this.lstAereas', this.lstAereas);
